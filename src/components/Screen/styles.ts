...
export const Wrapper = styled.div`
  ...
  width: 100%;
  height: 100%;
  cursor: url('../../../public/favicon.svg');
`;

export const Container = styled.div<{screen: { width: number, height: number }, showCursorObject: boolean}>`
  position: absolute;
  top: 0;
  left: 0;
  ...
  ${props => props.showCursorObject && css`
    cursor: crosshair;
  `}
`;

export const CursorObject = styled.div<{cursorObjectState: {positionX: number, positionY: number}}>`
  position: absolute;
