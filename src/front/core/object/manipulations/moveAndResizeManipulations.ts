...
function resizeUp(props: ManipulationPropties): ManipulationReturn {
  ...
  const { width } = props.size;
  const height =
    props.size.height + (props.position.y - props.cursorPosition.y);
...
function resizeLeftUp(props: ManipulationPropties): ManipulationReturn {
  ...
  const percentageIncreaseX = calcPercentageFromValue(
    props.size.width,
    advanceX
  );
  const percentageIncreaseY = calcPercentageFromValue(
    props.size.height,
    advanceY
  );
  ...
  const width = increaseByHighestPorcentage(
    percentageIncreaseX,
    percentageIncreaseY,
    props.size.width
  );
  const height = increaseByHighestPorcentage(
    percentageIncreaseX,
    percentageIncreaseY,
    props.size.height
  );
...
function increaseByHighestPorcentage(
  percentage1: number,
  percentage2: number,
  value: number
) {
...
export const manipulations: {
  [key: string]: (props: ManipulationPropties) => ManipulationReturn;
} = {
  move,
  resizeUp,
  resizeDown,
