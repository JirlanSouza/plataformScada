...
const addObject: CaseReducer<ScreensState, PayloadAction<AddObjectActionPayload>>  = (state, action) => {
  state[0].objects.items.push(new Object(
...
const manipulateObject: CaseReducer<ScreensState, PayloadAction<ManiputeObjectActionPayload>> = (state, action ) => {
...
const editObject: CaseReducer<ScreensState, PayloadAction<EditObjectActionPayload>> = (state, action) => {
...
const selectObject: CaseReducer<ScreensState, PayloadAction<{ id: number }>> = (state, action) => {
...
const unSelectObject: CaseReducer<ScreensState, PayloadAction<UnSelectObjectActionPayload>> =(state, action) => {
...
const editingProptiesObject: CaseReducer<ScreensState, PayloadAction<number>> = (state, action) => {
  state[0].objects.items[action.payload].editingPropties = true;
  state[0].objects.hasObjectsEditingsPropties = true;
}

const unEditingProptiesObject: CaseReducer<ScreensState, PayloadAction<number>> = (state, action) => {
